...
constexpr const char* TMP_PATH = "/tmp/RampageVPN.socket";
constexpr const char* VAR_PATH = "/var/run/RampageVPN/daemon.socket";
#endif
...
QString DaemonLocalServer::daemonPath() const {
  ...
  if (dir.exists("RampageVPN")) {
    logger.debug() << "/var/run/RampageVPN seems to be usable";
    return VAR_PATH;
  }

  if (!dir.mkdir("RampageVPN")) {
    logger.warning() << "Failed to create /var/run/RampageVPN";
    return TMP_PATH;
  }

  if (chmod("/var/run/RampageVPN", S_IRWXU | S_IRWXG | S_IRWXO) < 0) {
    logger.warning()
        << "Failed to set the right permissions to /var/run/RampageVPN";
    return TMP_PATH;
  }
